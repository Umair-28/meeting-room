...
@section('content')
<div class="container-fluid form-section p-3 mt-3 d-flex justify-content-center align-items-center" style="min-height: 100vh;">
    <div class="container main-section border p-4 rounded shadow" style="width:100%; max-width: 600px; background-color: rgba(255, 255, 255, 0.8);">
        <h4 class="text-center mb-4">Meeting Room Booking Form</h4>
        ...
        <form method="POST" action="{{ route('scheduleMeeting') }}" class="needs-validation" novalidate>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="title" value="{{ $meeting->title }}" class="form-control" id="meetingTitle" required>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control" value="{{ $meeting->organizer }}" name="organizer" id="organizer" required>
                    <div class="invalid-feedback">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control" value="{{ $meeting->date_time->format('Y-m-d') }}" name="date" id="meetingDate" required>
                    <div class="invalid-feedback">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="time" class="form-control" name="time" value="{{ $meeting->date_time->format('H:i') }}" id="meetingTime" required>
                    <div class="invalid-feedback">
                    ...
                    <select class="form-control select2" name="participants[]" id="participants" multiple="multiple" required>
                        @foreach($users as $user)
                            <option value="{{ $user->name }}" 
                                    @if(in_array($user->name, $meeting->participants)) selected @endif>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
...
<script>
    $(document).ready(function() {
        ...
        // Make sure the checkboxes are checked for preselected options
        $('#participants').on('select2:open', function() {
            $('li.select2-results__option[aria-selected=true]').each(function() {
                $(this).find('input[type=checkbox]').prop('checked', true);
            });
        });

        $('#participants').on('select2:select select2:unselect', function() {
            $('li.select2-results__option[aria-selected=true]').each(function() {
                $(this).find('input[type=checkbox]').prop('checked', true);
            });

            $('li.select2-results__option[aria-selected=false]').each(function() {
